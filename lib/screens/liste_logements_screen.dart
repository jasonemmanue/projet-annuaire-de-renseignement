...
class _ListeLogementsScreenState extends State<ListeLogementsScreen> {
  ...
  Future<void> _chargerPlus() async {
    ...
    // Simule un appel API (lazy loading §4.1.3)
    await Future.delayed(const Duration(milliseconds: 800));
    setState(() {
      _page++;
      _isLoading = false;
      if (_page > 3) _hasMore = false; // Fin des données mock
    });
  }

  List<Logement> get _logementsFiltrés {
...
Widget build(BuildContext context) {
  final logements = _logementsFiltrés;
...
Widget _buildListView(List<Logement> logements) {
  return ListView.builder(
    ...
    itemBuilder: (_, i) {
      // Publicité toutes les 5 annonces
      ...
      if (idx >= logements.length) {
        return _isLoading
          ...
          child: Center(child: CircularProgressIndicator(color: AppColors.primary)),
            ...
            : const Padding(
          padding: EdgeInsets.all(16),
          child: Center(child: Text('Plus de logements à charger', style: TextStyle(color: AppColors.textHint))),
        );
      }
      return sw.LogementCard(
        logement: logements[idx],
        onTap: () => Navigator.push(context, MaterialPageRoute(
            builder: (_) => DetailLogementScreen(logement: logements[idx]))),
      );
    },
  );
}
...
Widget _buildGridView(List<Logement> logements) {
  return GridView.builder(
    ...
    itemBuilder: (_, i) => sw.LogementCard(
      ...
      onTap: () => Navigator.push(context, MaterialPageRoute(
          builder: (_) => DetailLogementScreen(logement: logements[i]))),
      isGrid: true,
    ),
  );
}
...
// Données mock partagées (centralisées dans un vrai projet)
final List<Logement> _mockLogements = [
  Logement(
    id: '1', titre: 'Studio moderne à Bastos',
    description: 'Beau studio entièrement meublé avec accès internet.',
    ...
    equipements: ['Meublé', 'Wifi', 'Climatiseur'],
  ...
  Logement(
    id: '2', titre: 'Villa F4 à Bonanjo', description: 'Grande villa avec jardin.',
  ...
  Logement(
    id: '3', titre: 'Appartement F2 à Akwa', description: 'Appartement F2 au 3e étage.',
  ...
  Logement(
    id: '4', titre: 'Terrain à Mendong', description: 'Terrain de 500m² viabilisé.',
    prix: 8000000, typeLocation: 'vente', typeBien: 'Terrain',
    ville: 'Yaoundé', quartier: 'Mendong', latitude: 3.8200, longitude: 11.4900,
    photos: ['https://picsum.photos/seed/logement4/400/300'],
    surface: 500, nbPieces: 0,
    equipements: ['Titre foncier', 'Électricité', 'Eau'],
    estVerifie: true, estSponsorie: false,
